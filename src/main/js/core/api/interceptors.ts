...
import type { RequestInterceptor, ResponseInterceptor, RequestOptions } from './types';

/**
 * Authentication interceptor - adds auth token to requests
 */
export const authInterceptor: RequestInterceptor = (config: RequestOptions) => {
 ...
 */
export const loggingInterceptor: RequestInterceptor = (config: RequestOptions) => {
 ...
 */
export const errorInterceptor: ResponseInterceptor = async (response: Response) => {
  if (!response.ok) {
    // Log error details
 ...
 */
export const csrfInterceptor: RequestInterceptor = (config: RequestOptions) => {
 ...
 */
export const timestampInterceptor: RequestInterceptor = (config: RequestOptions) => {
  if (!config.params) {
    config.params = {};
  }
 ...
 */
export const timingInterceptor: ResponseInterceptor = async (response: Response) => {
  const timing = performance.getEntriesByName(response.url).pop() as PerformanceResourceTiming;
  ...
  if (timing) {
    ...
    (response as any).__timing = duration;
  }
 ...
 */
export const retryAfterInterceptor: ResponseInterceptor = async (response: Response) => {
  if (response.status === 429 || response.status === 503) {
    ...
    if (retryAfter) {
      ...
      (response as any).__retryAfter = retryDelay;
    }
  }
 ...
 */
export const contentTypeInterceptor: RequestInterceptor = (config: RequestOptions) => {
 ...
 */
export function createInterceptor<T extends RequestInterceptor | ResponseInterceptor>(
  fn: T
): T {
  return fn;
}