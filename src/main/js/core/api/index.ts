...
import { LuckyGasAPI } from './endpoints';
import { 
  authInterceptor, 
  loggingInterceptor, 
  errorInterceptor,
...
// Export interceptors for custom use
...
window.addEventListener('api:servererror', (event: any) => {
...
window.addEventListener('api:ratelimited', (event: any) => {
...
(window as any).luckyGasAPI = api;
(window as any).apiLoadingManager = loadingManager;